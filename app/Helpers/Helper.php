...
{
    try {
        // Define the path to store the certificate in 'public/uploads/certificates'
        $certificatePath = public_path('uploads/certificates');
        
        // Check if the directory exists, if not, create it
        if (!file_exists($certificatePath)) {
            mkdir($certificatePath, 0777, true);  // Create directory with proper permissions
        }
        ...
        // Define the full path for saving the certificate
        $certificateFilePath = $certificatePath . '/' . $certificateFileName;

        // Save the generated PDF to the specified path
        $pdf->save($certificateFilePath);
